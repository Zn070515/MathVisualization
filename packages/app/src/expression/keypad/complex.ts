 ...
 * insert anything: a key that produced `\oint` would produce a line the parser
 * rejects, and a key that returned a number would be inventing an answer. The
 * mathematics behind each is recorded in the capability list, where its status is
 * stated in full.
 */
export const COMPLEX_PLANNED_ROWS: readonly KeypadRow[] = [
  { kind: 'heading', title: 'Not yet in the expression language' },
  {
    ...
    keys: [
      ...
      plannedKey('∮', 'contour integration'),
      plannedKey('Σ', 'series'),
